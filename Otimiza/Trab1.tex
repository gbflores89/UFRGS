...
\end{equation}
Para a Equação \autoref{eq:1f1} temos que o gradiente de $f_1(x)$ é dado por:
\begin{equation}
\nabla f_1(x) = \left[
\begin{array}{c}
2\\
...
\right.$
\end{itemize}
portanto temos uma função convexa para valores positivos de $x$ e não convexa
...
\end{equation}
Para a Equação \autoref{eq:2f1} temos que o gradiente de $f_1(x)$ é dado por:
\begin{equation}
\nabla f_1(x) = \left[
\begin{array}{cc}
1 &\quad \text{se } x> 0\\
...
aplicados em $f_1(x)$ são obtidos valores maiores que $0$, como podemos observar
na \autoref{fig:2f1}:
\begin{figure}[h]
\centering
...
Portanto podemos concluir que temos um ponto de mínimo, apesar do gradiente de
$f_1(x)$ não ser contínuo e portanto nao ser possível o calculo da hessiana de
$f_1(x)$.
...
observando na \autoref{fig:2f2} onde é apresentado o comportamento da função
$f_2(x)$.
...
que se trata de um ponto de máximo, para este caso.
