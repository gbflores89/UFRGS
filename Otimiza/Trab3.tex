...
Como é possível observar nas Tabelas \ref{tab:res1} e \ref{tab:res2} o
algoritmo \code{hkjeeves} apresentou o melhor resultado entre os algoritmos
testados ficando significativamente à frente do segundo colocado, o algoritmo
\code{cgrad}. Apesar de bastante claros, os resultados obtidos são bastante
contraditórios em primeiro momento, pois demonstra que os métodos mais simples
são melhores que os métodos que utilizam mais informação sobre o problema
(\emph{i.e.} gradiente e hessiana), como por exemplo o algoritmo
\code{lmarqua}. Na \autoref{tab:resBoleana} estão apresentados, de forma
\emph{boleana} quais problemas cada método foi capaz de resolver na política de
chutes inicias com valores entre $[0\;\ 2]$. Esta ultima tabela ajuda a elucidar os
resultados pouco satisfatório para métodos melhores elaborados, presentes nas
