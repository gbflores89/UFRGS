...
\boxexercise{6.48 (EHL, \citeyear{edgar2001optimization})}
...
\autoref{code:ehl648script1} \lstinputlisting[language=Matlab, caption={Script de execução},
label={code:ehl648script1}]{trab2/EHL-6-48/Script1.m}

\clearpage
Os resultados para cada interação:
\VerbatimInput[firstline=2, lastline=19]{trab2/EHL-6-48/Resultado1.txt}
...
Os resultados para o método de Newton foram obtidos com o
\autoref{code:ehl648script2} \lstinputlisting[language=Matlab, caption={Script de execução}, 
label={code:ehl648script2}]{trab2/EHL-6-48/Script2.m} 

Os resultados para cada interação:
\VerbatimInput[firstline=5, lastline=8]{trab2/EHL-6-48/Resultado2.txt}
...
\autoref{eq:ehl644} devemos encontar um melhor valor para $F(b)$.
\begin{equation}
\label{eq:ehl644f}
...
\right)
\end{equation}
onde a função $\erf$ é tabelada e dada por:
\begin{equation}
\erf\left(x\right) = \frac{2}{\sqrt{\pi}}\int_0^x \exp{\left(-t^2\right)}\diff t
\end{equation}

Assim, é facilmente obtida aexpressão de $\varepsilon\{risk\}$, como
segue:
